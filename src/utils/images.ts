...
const OL_RE =
  /covers\.openlibrary\.org\/b\/(?:isbn|id)\/([^./]+)-([SML])\.jpg(?:.*)?$/i;

export function isOpenLibraryUrl(url?: string): boolean {
  return !!url && OL_RE.test(url);
}

export function toOpenLibVariant(
  url: string,
  size: 'S' | 'M' | 'L' = 'L',
): string {
  return url.replace(
    OL_RE,
    (_m, id) => `covers.openlibrary.org/b/isbn/${id}-${size}.jpg`,
  );
}

export function buildSrcSet(
  url?: string,
  kind: 'thumb' | 'hero' = 'thumb',
...
export function posterFor(b: Book) {
  return (
    b.image_url ||
    `https://placehold.co/1200x360?text=${encodeURIComponent(b.title)}`
  );
}
