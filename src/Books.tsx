...
import BookCardPlaceholder from './components/books/BookCardPlaceholder';
...
const BooksAxios: React.FC = () => {
      ...
      <div className="space-y-4 px-4 sm:px-6 lg:px-10">
        ...
        <div className="mt-2 flex gap-4">
          {Array.from({ length: perPage }).map((_, i) => (
            <div
              key={i}
              className="h-[260px] w-[180px] animate-pulse rounded-2xl bg-white/10"
            />
          ))}
        </div>
      </div>
  ...
  if (error) {
    return <p className="px-4 sm:px-6 lg:px-10 text-red-300">{error}</p>;
  }
